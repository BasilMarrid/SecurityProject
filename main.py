...
def get_reg_mappings(inscode, reg_map, counter, allregs):
    reg_map = {} if reg_map is None else reg_map
    instructs = inscode.split()
    for ins in instructs:
        operands = ins.split("|")
        for oper in operands:
            if oper in allregs and oper not in reg_map:
                reg_map[oper] = f"R_{next(counter)}"
    return reg_map
...
def con_to_str(con, replace_strs=[', ', ' ', '(', ')'], max_depth=8):
    ...
    for r_str in replace_strs:
        repr = repr.replace("\s","").replace("<","").replace(">","").replace(r_str, '|')
...
def gen_new_name(old_name):
    ...
    if re.match(r"strlen", old_name):
        return re.sub("(_[0-9]+)+", '', old_name[len("strlen") : ])
    if re.match(r"strncmp_ret", old_name):
        return re.sub("(_[0-9]+)+", '', old_name[len("strncmp_ret") : ])
    return old_name


def varify_cons(cons, var_map=None, max_depth=8):
    """
...
def sm_to_output(sm: angr.sim_manager.SimulationManager, output_file, func_name):
    ...
    allregs = ["rax","rip","rcx","rdx","rbx","rsi","rdi","rsp","rbp","r8","r9","r10","r11","r12","r13","r14","r15","eax","ecx","edx","ebx","esi","edi","esp","ebp","r8d","r9d","r10d","r11d","r12d","r13d","r14d","r15d","ax","cx","dx","bx","si","di","sp","bp","r8w","r9w","r10w","r11w","r12w","r13w","r14w","r15w","cl","dl","bl","sil","dil","spl","bpl","r8b","r9b","r10b","r11b","r12b","r13b","r14b","r15b"]
    reg_map = dict()
    skipped_lines = 0
    proj = sm._project
    for exec_paths in sm.stashes.values():
        for exec_path in exec_paths:
            ...
            reg_map=get_reg_mappings(processed_code,reg_map=reg_map,counter=counter,allregs=allregs)
            var_map, relified_consts = varify_cons(exec_path.solver.constraints, var_map=var_map)
            relified_consts = "|".join(relified_consts)
            relified_consts = relified_consts.replace("reg_","")
            relified_consts = remove_consecutive_pipes(relified_consts)
            line = f"{tokenize_function_name(func_name)} DUM,{processed_code}|CONS|{relified_consts},DUM\n"
            found_constants = list(re.findall(r"0[xX][0-9a-fA-F]+", line))
            found_underscores = set(re.findall(r"[_]+", line))
            for constant in sorted(found_constants,reverse=True,key=lambda x: len(x)):
                line=line.replace(constant,"C")
            line = updateNumericalRegs(line)
            line = line.replace("*","|*|")
            found_numbers = list(re.findall(r"[0-9]+",line))
            for found_number in sorted(found_numbers,reverse=True,key=lambda x: len(x)):
                line=line.replace(found_number,"N")
        #    for regi in allregs:
        #        if regi in reg_map:
        #            line=line.replace(regi,reg_map[regi])
            found_underscores = set(re.findall(r"[_]+", line))
            for us in found_underscores:
                line=line.replace(us,"")
            line = line.replace("|...|","")
            line = line.replace("...","")
            line=line.replace("qword","Q")
            line=line.replace("dword","D")
            line=line.replace("byte","B")
            line=line.replace("ptr","P")
            if len(line) <= 3000:
...
def updateNumericalRegs(line):
    line = line.replace("r8d","rEigd")
    line = line.replace("r8w","rEigw")
    line = line.replace("r8b","rEigb")
    line = line.replace("r8","rEig")

    line = line.replace("r9d","rNind")
    line = line.replace("r9w","rNinw")
    line = line.replace("r9b","rNinb")
    line = line.replace("r9","rNin")

    line = line.replace("r10d","rTend")
    line = line.replace("r10w","rTenw")
    line = line.replace("r10b","rTenb")
    line = line.replace("r10","rTen")

    line= line.replace("r11b","rEleb")
    line= line.replace("r11w","rElew")
    line= line.replace("r11d","rEled")
    line= line.replace("r11","rEle")

    line= line.replace("r12b","rTweb")
    line= line.replace("r12d","rTwed")
    line= line.replace("r12w","rTwew")
    line= line.replace("r12","rTwe")

    line=line.replace("r13d","rThird")
    line=line.replace("r13w","rThirw")
    line=line.replace("r13b","rThirb")
    line=line.replace("r13","rThir")

    line = line.replace("r14b","rFourb")
    line = line.replace("r14d","rFourd")
    line = line.replace("r14w","rFourw")
    line = line.replace("r14","rFour")

    line = line.replace("r15b","rFifb")
    line = line.replace("r15w","rFifw")
    line = line.replace("r15d","rFifd")
    line = line.replace("r15","rFif")
    return line
...
def generate_output(dataset_path):
    ...
    val_output = open(os.path.join(dataset_path, "valid_output.txt"), "w")
    mapper = dict()
    all_funcs = set()
